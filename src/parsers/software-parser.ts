...
function getSoftwareProcessApplicationFields(softwareProcess: any) {
    const applicationFieldsList = getFirstElement(softwareProcess?.campoAplicacaoLista);

    if (isNonEmptyArray(applicationFieldsList?.campoAplicacao)) {
        return applicationFieldsList?.campoAplicacao.reduce((acc: any[], campoAplicacaoAtual: any) => {
